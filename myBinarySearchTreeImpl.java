...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
    ...
    public int my_count_at_level(int level){
        if (root==null||level<1)
            return 0;
        //only 1 node at this level(root node)
        if (level==1)
            return 1;
        //check left + right tree with decremented level
        //go down tree until node at level 1 is found
        int c1=my_left_tree().my_count_at_level(level-1);
        int c2=my_right_tree().my_count_at_level(level-1);
        //sum of them
        return c1+c2;
    }
    ...
    public boolean my_is_balanced(){
        if (my_is_empty())
            return true;
        myBinarySearchTree<T1,T2> left_tree=my_left_tree();
        myBinarySearchTree<T1,T2> right_tree=my_right_tree();
        //1.check difference between subtrees must be of max 1
        //2.left + right subtree is balanced
        if(Math.abs(left_tree.my_length()-right_tree.my_length())<=1 && left_tree.my_is_balanced()&&right_tree.my_is_balanced())
            return  true;
    ...
    public int my_count_smaller_nodes(T1 key){
        if(my_is_empty())
            return 0;
        //check if current node is smaller than key
        int smaller_node=0;
        //if smaller, set variable to 1
        if(root.getKey().compareTo(key)<0)
            smaller_node=1;
        //return smaller nodes in subtrees plus 1 if this node is smaller
        return smaller_node+my_left_tree().my_count_smaller_nodes(key)+my_right_tree().my_count_smaller_nodes(key);
    }
    ...
    public int my_find_node_at_level(T1 key){
        if(my_is_empty())
            return 0;
        //compare given key to the key of current tree root
        //returns 0 if equal, less than 0 if smaller, more than 0 if greater
        int k=key.compareTo(this.my_root().getKey());
        //if current tree root is equal to key, return 1
        if(k==0)
            return 1;

        //left and right tree
        int left = my_left_tree().my_find_node_at_level(key);
        int right = my_right_tree().my_find_node_at_level(key);

        //if found in any node, 1 eventually returned
        //if returned 1, return it add 1 and go up stack
        //method returned 1+1, upper called method will return 1+1+1
        //add as many 1's as the level of the node
        if(left>0||right>0)
            return 1+left+right;
